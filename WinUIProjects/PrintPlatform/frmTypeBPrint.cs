...
{
    public partial class frmTypeBPrint : ChildContentForm, IChildForm, IChildModel<RecordsBaseInfoModel>
    {
        ...
        private void frmTypeBPrint_Load(object sender, EventArgs e)
        {
        ...
        {
            if (((this.dgvData.Columns[e.ColumnIndex].HeaderText == "B超") && (this.bds.Position >= 0)) && (e.RowIndex >= 0))
            {
                DataRowView view = (DataRowView)this.bds.List[this.bds.Position];
                if (this.dgvData[e.ColumnIndex, this.bds.Position].Value.ToString() == "B超")
                {
                    TypeBShow data = new TypeBShow
                    {
                        FilePath = string.Format(BChaoReport + @"\{0}.jpg ", view.Row["MID"])
                    };
