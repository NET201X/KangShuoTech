...
namespace RecordManagement.OverView
{
    ...
    public partial class frmOutHyper : ChildContentForm, IChildForm, IChildModel<RecordsBaseInfoModel>
    {
        ...
        public frmOutHyper()
        {
            ...
            requireBaseDt = RequireBLL.GetList(" TabName = '高血压随访' AND Comment = '高血压基本信息' AND IsRequired='1' ").Tables[0];
            requireVisitDt = RequireBLL.GetList(" TabName = '高血压随访' AND Comment = '高血压随访信息' AND IsRequired='1' ").Tables[0];
            ...
            if (dtVisit.Rows.Count > 0)
            {
                ...
                for (int i = 0; i < dtVisit.Rows.Count; i++)
                {
                    ...
                    else
                    {
                        ...
                        // sum必输栏位空白的笔数
                        reqVisitCol += " (CASE WHEN IFNULL(" + tableName + col + ", '')='' THEN 1 ELSE 0 END) +";
                        reqVisitTipCol += " CASE WHEN IFNULL(" + tableName + col + ", '')='' THEN '" + colName + "\\r\\n' ELSE '' END,";
                    }
                }
        ...
        private void GetData()
        {
            ...
            else
            {
                ...
                // 取得清单总笔数
                this.totalCount = archive_baseinfo.GetHyperDataCount(reqBaseCol + reqVisitCol, where + reqWhere);
        ...
        private void frmOutHyper_Load(object sender, EventArgs e)
        {
        ...
        private string GetWhere()
        {
            ...
            str2 = str2 + string.Format("AND Base.PopulationType LIKE '%{0}%' ", "6");
            if (this.ckCheckDate.Checked)
            {
                str2 = str2 + string.Format(" AND Visit.FollowUpDate BETWEEN '{0}' AND '{1}' ",
            ...
            {
                str2 = str2 + string.Format(" AND Visit.FollowUpDoctor = '{0}' ", this.cboDoctor.Text.ToString()); ;
            }
        ...
        private void dgvData_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if ((this.bds.Position >= 0) && (e.RowIndex >= 0))
            {
                ...
                {
                    using (Controler controler = new Controler(new MDIParentForm(idNo), new HypFactory(1)))
                    {
                ...
                {
                    using (Controler controler = new Controler(new MDIParentForm(idNo), new HypFactory(2)))
                    {
                        controler.IParentFrm.IShowDialog();
                        GetData();
                        return;
